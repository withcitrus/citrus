...
self.addEventListener('push', function(event) {
  ...
  // Check if the user is looking at Citrus right now and don't show a notification
  event.waitUntil(
    clients
      .matchAll({
        type: 'window',
      ...
      .then(windowClients => {
        for (let i = 0; i < windowClients.length; i++) {
          ...
          // The user is looking at Citrus right now abort showing the notification!
  ...
  event.waitUntil(
    ...
    self.clients
      ...
      .then(function(clientList) {
        // If there is an open citrusapp.chat window navigate to the notification href
        if (clientList.length > 0) {
          return clientList[0]
            .focus()
            .then(client => client.navigate(urlToOpen));
        }
        // If there's no open citrusapp.chat window open a new one
        // eslint-disable-next-line
